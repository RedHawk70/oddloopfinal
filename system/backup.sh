...
clear
echo " VPS Data Backup By NiLphreakz "
sleep 1
...
cd /root
zip -r $IP-$date-$domain-oddloop.zip backup > /dev/null 2>&1
rclone copy /root/$IP-$date-$domain-oddloop.zip dr:backup/
url=$(rclone link dr:backup/$IP-$date-$domain-oddloop.zip)
...
rm -rf /root/backup
rm -r /root/$IP-$date-$domain-oddloop.zip
echo ""
