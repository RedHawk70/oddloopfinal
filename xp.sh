...
for user in "${data[@]}"
do
exp=$(grep -w "^#vms $user" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 3 | sort | uniq)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" -le "0" ]]; then
...
sed -i "/^#vls-http $user $exp/,/^},{/d" /usr/local/etc/xray/config.json
sed -i "/^#vls-http $user $exp/,/^},{/d" /usr/local/etc/xray/none.json

sed -i "/^#vls-xhttp $user $exp/,/^},{/d" /usr/local/etc/xray/config.json
sed -i "/^#vls-xhttp $user $exp/,/^},{/d" /usr/local/etc/xray/none.json
...
#----- Auto Remove Trojan
data=( `cat /usr/local/etc/xray/config.json | grep '^#trws' | cut -d ' ' -f 2 | sort | uniq`);
now=`date +"%Y-%m-%d"`
for user in "${data[@]}"
do
exp=$(grep -w "^#trws $user" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 3 | sort | uniq)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
if [[ "$exp2" -le "0" ]]; then
sed -i "/^#trws $user $exp/,/^},{/d" /usr/local/etc/xray/config.json
sed -i "/^#trws $user $exp/,/^},{/d" /usr/local/etc/xray/none.json
systemctl restart xray@config
systemctl restart xray@none
fi
done
